...
pub fn hide_cm() -> bool {
    // This build never shows a local UI: the connection-manager window must never be
    // painted, regardless of approve-mode/verification-method/allow-hide-cm config.
    // Practical implication: approve-mode must be "password" for incoming connections
    // to ever get approved, since there is no window left to click "accept" on.
    true
}
